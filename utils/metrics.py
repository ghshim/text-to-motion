...
def check_covariance_matrix(sigma1, sigma2):
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))

    axes[0].imshow(sigma1, cmap="viridis", aspect="auto")
    axes[0].set_title("Sigma1 (Generated Motion)")

    axes[1].imshow(sigma2, cmap="viridis", aspect="auto")
    axes[1].set_title("Sigma2 (Real Motion)")

    plt.colorbar(axes[0].imshow(sigma1, cmap="viridis", aspect="auto"), ax=axes[0])
    plt.colorbar(axes[1].imshow(sigma2, cmap="viridis", aspect="auto"), ax=axes[1])

    plt.show()

    print("Sigma1: min =", np.min(sigma1), ", max =", np.max(sigma1))
    print("Sigma2: min =", np.min(sigma2), ", max =", np.max(sigma2))

def check_mean_vector(mu1, mu2):
    plt.figure(figsize=(10, 5))
    plt.plot(mu1, label="Generated Motion (mu1)", alpha=0.7)
    plt.plot(mu2, label="Real Motion (mu2)", alpha=0.7)
    plt.legend()
    plt.title("Comparison of Mean Vectors")
    plt.show()